...
class XodEYEv:
    ...
    def xodLinEFFX(self, imgSeqArray, numFrames, xFrames, srcReshape,
                   effx, fadeInOut, fwdRev, n_offset, n_digit, eyeOutDir, eyeOutFileNm):
        ...
        for i in range(xBeats):
            ...
            self.xodImgLinEFFX(imgSeqArray[offsetIdx:len(imgSeqArray)], xFrames, ctrl, effx, srcReshape,
                               fadeInOut, fwdRev, new_offset, n_digit, eyeOutDir, eyeOutFileNm)
    ...
    def xodImgLinEFFX(self, imgFileList, numFrames, ctrl, effx, srcReshape, fadeInOut, fwdRev,
        ...
        for i in range(numFrames):
            ...
            if effx == 2:
                ...
                # RGB rotate..
                imgRgbRot = eyeutil.xodColorRotate(imgPIL1, alphaX[i])
    ...
    def xodSegmentEFFX(self, segSeqArray, imgSeqArray, numFrames, xFrames, effx,
        ...
        for i in range(xBeats):
            ...
            new_offset = n_offset + i * xFramesAlt
            ...
            self.xodImgSegmentEFFX(segSeqArray, imgSeqArray, xFramesAlt, xFramesAlt, effx,
                                   srcReshape, offsetIdx, new_offset, n_digits, eyeOutDir, eyeOutFileNm)
    ...
    def xodImgSegmentEFFX(self, segSeqArray, imgSeqArray, numFrames, xFrames, effx,
        ...
        if len(imgSeqArray) < numSegments:
            print("ERROR: imgSeqArray must contain at least " + str(numSegments) + " sequences")
            sys.exit()
        ...
        for i in range(numFrames):
            ...
            maskSrc = imio.imread(segSeqArray[maskSel][eyeutil.circ_idx(i + offsetIdx + n_offset,
                                                                        len(segSeqArray[maskSel]))])
            ...
            if effx == 0:
                ...
                eyeSrcArray = []
                for j in range(numSegments):
                    imgTmp = imio.imread(imgSeqArray[seqSel[j]][eyeutil.circ_idx(i + offsetIdx + n_offset,
                                                                                 len(imgSeqArray[seqSel[j]]))])
                    if srcReshape:
                        imgPILtmp = Image.fromarray(imgTmp)
                        imgPILtmp = eyeutil.xodEyeReshape(imgPILtmp, self.mstrSzX, self.mstrSzY, 0)
                        imgTmp = np.asarray(imgPILtmp)
                ...
                for j in range(numSegments):
                    if fwdRev == 2:
                        imgTmp = imio.imread(
                            imgSeqArray[seqSel[j]][eyeutil.circ_idx((numFrames - 1 - i) + offsetIdx + n_offset,
                                                                    len(imgSeqArray[seqSel[j]]))])
                    else:
                        imgTmp = imio.imread(imgSeqArray[seqSel[j]][eyeutil.circ_idx(i + offsetIdx + n_offset,
                                                                                     len(imgSeqArray[seqSel[j]]))])
            ...
            for t in range(framesPerBeat):
                ...
                for j in range(SzY):
                    for k in range(SzX):
                        ...
                        if ((j >= (t + 1) * hop_sz) and (j < newDimY + ((t + 1) * hop_sz) / 2) and (
                                k >= (t + 1) * hop_sz) and (k < newDimX + ((t + 1) * hop_sz) / 2)):
    ...
    def odmkEyeEchoBpm(self, imgList, numFrames, echoTrig, numEcho, echoStep, imgOutDir, echoDecay='None',
                       imgOutNm='None'):
                ...
                if frameCnt < numFrames:  # process until end of total length
                    ...
                    for j in range(SzY):
                        ...
                        for k in range(SzX):
                            ...
                            if ((j >= (SzY - newDimY) / 2) and (j < (SzY - (SzY - newDimY) / 2)) and (
                                    k >= (SzX - newDimX) / 2) and (k < (SzX - (SzX - newDimX) / 2))):
    ...
    def odmkBSlothRecurs(self, imgArray, xLength, framesPerSec, xfadeFrames, imgOutDir, imgOutNm='None',
                         indexOffset='None'):
                    ...
                    for j in range(SzY):
                        for k in range(SzX):
                            ...
                            if ((j >= (SzY - newDimY) / 2) and (j < (SzY - (SzY - newDimY) / 2)) and (
                                ...
                                if ((j + offsetY >= 0) and (j + offsetY < SzY) and (k + offsetX >= 0) and (
                                        k + offsetX < SzX)):
                                    # print('*****j = '+str(j)+'; k = '+str(k)+'; j+offsetY = '+str(j+offsetY)+'; k+offsetX = '+str(k+offsetX))
                                    imgBpTsc1[j + offsetY, k + offsetX, :] = imgItr1[int(j - (SzY - newDimY) / 2),
                                                                             int(k - (SzX - newDimX) / 2), :]
                                    imgBpTsc2[j + offsetY, k + offsetX, :] = imgItr2[int(j - (SzY - newDimY) / 2),
                                                                             int(k - (SzX - newDimX) / 2), :]
    ...
    def odmkBSlothGlitch1(self, imgArray, xLength, framesPerSec, xfadeFrames, imgOutDir, imgOutNm='None', inOrOut=0,
                          indexOffset='None'):
                    ...
                    for j in range(SzY):
                        for k in range(SzX):
                            ...
                            if ((j >= (t + 1) * hop_sz) and (j < newDimY + ((t + 1) * hop_sz) / 2) and (
                                ...
                                # imgBpTsc1[j, k, :] = imgItr1[int(j - (SzY-newDimY)/2), int(k - (SzX-newDimX)/2), :]
                                # imgBpTsc2[j, k, :] = imgItr2[int(j - (SzY-newDimY)/2), int(k - (SzX-newDimX)/2), :]
                                if (j + offsetY >= 0 and j + offsetY < SzY) and (
                                        k + offsetX >= 0 and k + offsetX < SzX):
                                    # print('*****j = '+str(j)+'; k = '+str(k)+'; j+offsetY = '+str(j+offsetY)+'; k+offsetX = '+str(k+offsetX))
                                    imgBpTsc1[j + offsetY, k + offsetX, :] = imgItr2[int(j - (SzY - newDimY) / 2),
                                                                             int(k - (SzX - newDimX) / 2), :]
                                    imgBpTsc2[j + offsetY, k + offsetX, :] = imgItr1[int(j - (SzY - newDimY) / 2),
                                                                             int(k - (SzX - newDimX) / 2), :]
