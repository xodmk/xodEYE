...
class XodEYEu:
    ...
    # // *********************************************************************** //    
    # // *********************************************************************** //
    # // *---:: XODMK img Post-processing func::---*
    # // *********************************************************************** //
    # // *********************************************************************** //

    # // *--------------------------------------------------------------* //
    # // *---:: XODMKEYE - interlace img files in two directoroies::---*
    # // *--------------------------------------------------------------* //
    ...
    def xodImgTelescope(self, imgList, xLength, framesPerSec, framesPerBeat, imgOutDir, imgOutNm='None', inOrOut=0):
        ...
        for i in range(numBeats):
            ...
            for t in range(framesPerBeat):
                ...
                for j in range(SzY):
                    for k in range(SzX):
                        # if ((j >= (t+1)*hop_sz) and (j < (newDimY+(SzY-newDimY)/2)) and
                        #       (k >= (t+1)*hop_sz) and (k < (newDimX+(SzX-newDimX)/2))):
...
#            qtrcos = eye.quarterCos(numEcho)    # returns a quarter period cosine wav of length n    
    ...
    # // *--------------------------------------------------------------* //
    # // *---:: XODMKEYE - EYE Pixel Random Replace::---*
    # // *--------------------------------------------------------------* //
...
#                offsetY = int(focalRnd1[1] - SzY/2)
            ...
            for t in range(xfadeFrames):
                if frameCnt < numFrames:    # process until end of total length
                    ...
                    imgItr3 = self.odmkEyeRescale(imio.imread(imgFileList[round(numImg*random.random()) % numImg]),
                                                  newDimX, newDimY)
                    ...
                    # imgBpTscOut = self.eyeSubInsert(imgBpTsc1, imgBpTsc2, imgSub1, imgSub2, SzX, SzY,
                    #                                 newDimX, newDimY, alphaX[t])
                    imgBpTscOut = self.eyeSubInsert(imgClone1, imgClone2, imgSub1, imgSub2, SzX, SzY,
                                                    newDimX, newDimY, alphaX[t])
